...
const getActualBlogPosts = (): BlogPost[] => {
  ...
  // Default content if no saved posts
  return [
    {
      id: "1",
    ...
    <Card className={cn(
      ...
      <div className={cn(
        ...
        <div className={cn(
          ...
          {article.contentType && (
            <div className="absolute top-2 right-2 bg-byzantine/80 text-white text-xs px-3 py-1 rounded shadow">
              {article.contentType}
            </div>
          )}
        </div>
        <CardContent className="p-4 flex flex-col h-full">
          ...
          <div className="mt-auto pt-4">
            <Button asChild variant="link" className="p-0 h-auto font-medium text-byzantine hover:text-byzantine-dark">
              <Link to={`/content-library/${article.id}`} className="flex items-center gap-1">
...
export function FeaturedArticles() {
  ...
  useEffect(() => {
    // Load actual articles from localStorage
    setArticles(getActualBlogPosts());
  }, []);
    ...
    <section className="py-10 md:py-14">
      <div className="container mx-auto px-4">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 gap-4">
          <div>
            <h2 className="orthodox-heading text-2xl md:text-3xl font-extrabold mb-2 text-gold/95 glow-shadow">Explore Our Content</h2>
        ...
        <div className="mt-10 text-center">
          <Button asChild variant="outline" className="border-gold/60 text-gold hover:bg-gold/10 shadow-md rounded-lg px-6 font-semibold">
            <Link to="/content-library">View All Content</Link>
          </Button>
        </div>
      </div>
    </section>
